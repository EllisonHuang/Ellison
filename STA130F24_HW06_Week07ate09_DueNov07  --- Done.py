...
# A simple linear regression model is the linear relationship between X(the predictor variable/independent variable) and Y(outcome variables/dependent variable) using beta1(the slope/rate of change or the expected change for the next unit of x), beta0(intercept coefficients/telling the start of Y when X is at zero) and sigma(error term/All the y values that are not on the line). 
# 
# To form a sample we place all the value of X inside the formula Y = β0 + β1X + ϵ to find all the Y values and placing it on a graph with a line defineded by Y = β0 + β1X + ϵ.
...
x_start = 0         # Start value for X
x_end = 10          # End value for X
beta0 = 5          # Intercept (beta0)
beta1 = 2.5        # Slope (beta1)
...
# Calculate Y values based on the theoretical model
Y = beta0 + beta1 * X + errors

# Create a DataFrame to hold the data
df = pd.DataFrame({'x': X, 'Y': Y, 'Data': 'Observed'})

# Scatter plot with a trendline using plotly.express
fig = px.scatter(df, x='x', y='Y', color='Data', trendline='ols', title='Y vs. x with Trendline')
fig.show(renderer="png")


# Chat Link: https://chatgpt.com/share/672c3646-f1d4-8013-8caf-93d97b3060d8
...
import statsmodels.formula.api as smf  # This allows us to write a formula where the dependent 
                                       # variable and independent variables are specified.
import plotly.express as px  # this is a ploting library


# First indicate that the model will look at df with the dependent variable Y and the independent variable X. 
# Using the data given the slope is stored as fitted_model.
model_data_specification = smf.ols("Y~x", data=df) 
fitted_model = model_data_specification.fit() 

# what do each of the following provide?
fitted_model.summary()  # This provides the Model Information, R-squared and Adjusted R-squared, 
                        # F-statistic and Prob (F-statistic), Coefficient Table and Diagnostic Information of fitted model.
fitted_model.summary().tables[1]  # This is showing summary of  Coefficient Table including Variable Names, Coefficient, 
                                  # Standard Error, p-value and Confidence Interval.
fitted_model.params  # It uses the independent Variables X to estimate dependent Variables Y using Intercept and slope.
fitted_model.params.values  # Uses independent Variables X and  dependent Variables Y to estimate Intercept and slope.
fitted_model.rsquared  # This returns the R-squared value of the regression model, that is telling how high i can 
                       # how much I can trust the model to explain the variability in Y.

# Creates a column called Data so the data point is used alone. 
# Adding a trend line to show the linear relationship between X and Y
df['Data'] = 'Data' # hack to add data to legend 
fig = px.scatter(df, x='x',  y='Y', color='Data', 
                 trendline='ols', title='Y vs. x')
...
# Chat link: https://chatgpt.com/share/672d6101-017c-8013-9fac-f1fa56591bcc
...
# The added line tells the true model parameters that the Y should originally be on. The lines in questions 1 and 2 estimate the line not the actual line.
...
# chat link: https://chatgpt.com/share/672d770e-2660-8013-aa58-692c7e7b30d1
...
# fitted_model.params uses an error term as small as possible to create the fitted simple linear regression models. On the other hand, theoretical uses the beta given to provide an actual error and then find the simple linear regression models.

# chat link: https://chatgpt.com/share/672d7a20-ebe4-8013-bd5a-c261fde5c2c9
...
# The line chosen is to minimize the sum of the squared differences between the observed data points and the predicted values on the line. The reason for using squares is to provide positive residuals, if not the positive and negative residuals could cancel each other out giving a false result. Squares also always OLS to give out a clear outliner as it penalizes larger deviations. 
...
# 1. ((Y-fitted_model.fittedvalues)**2).sum() find Y that is not explained by the model. ((Y-Y.mean())**2).sum() find total of the Y(independent variable). This is calculating what is the option that is not explained and when this is minus from one you'll be able to get a portion unexplained. When R^2 is equal to zero that means none of the variance are explained. 
# 
# 2. fitted_model. squared provides a direct indication of how well the model explains the variability in the outcome variable Y. This is also saying the higher the R^2 the more accurate the model in terms of fitting the data. 
# 
# 3. np.corrcoef(Y,fitted_model.fittedvalues)[0,1]**2 is just another way to calculate R^2.
# 
# 4. np.corrcoef(Y,x)[0,1]**2 provide how the x could be found Y by itself so it could provide a predictor in simple linear regression.

# chat link: https://chatgpt.com/share/672d7ec4-f5cc-8013-8f97-66b628b4a04b
...
# 1. The increasing of crops is not linear with the increasing of fertilizer. 
# 
# 2. The residuals tend to vary more as the amount of fertilizer increases, which suggests heteroscedasticity.

# chat link: https://chatgpt.com/share/672d808b-8338-8013-98b0-0a26be59c20f
...
# the null hypothesis of "no linear association (on average)" 
# This suggests the waiting does not effect the overall data set 
# looking at the P>|t| is 0.000 saying is strongly rejects the Null Hypothesis.
...
# Looking at the code for the three we could P>|t| is increasing for a shorter time. Also looking at the coefficient we could see is between 95% confidence interval. This suggests there is weaker evidence against the null hypothesis when the time is shorter.
...
bootstrap_slopes_df = pd.DataFrame(bootstrap_slopes, columns=['Slope Coefficients'])

# Plot the distribution of slope coefficients
fig = px.histogram(bootstrap_slopes_df, x='Slope Coefficients', nbins=30,
...
# Given parameters for the null hypothesis model
beta_0 = 1.65
beta_1 = 0
sigma = 0.37
n_simulations = 1000  # Number of simulations

# Simulated sampling distribution of slope coefficients
simulated_slopes = []

# Use waiting times as input
waiting_times = old_faithful['waiting']

for _ in range(n_simulations):
    # Simulate Y under the null hypothesis
    simulated_Y = beta_0 + beta_1 * waiting_times + np.random.normal(0, sigma, len(waiting_times))
    
    # Create a DataFrame with simulated data
    simulated_data = pd.DataFrame({'waiting': waiting_times, 'duration': simulated_Y})
    
    # Fit the regression model and record the slope coefficient
    model = smf.ols('duration ~ waiting', data=simulated_data).fit()
    simulated_slopes.append(model.params['waiting'])

# Convert to DataFrame for easy plotting
simulated_slopes_df = pd.DataFrame(simulated_slopes, columns=['Slope Coefficients'])

# Plot the distribution of slope coefficients
fig = px.histogram(simulated_slopes_df, x='Slope Coefficients', nbins=30,
...
# The bootstrapped p-value is the proportion of bootstrap samples where the slope coefficient was less than or equal to zero
p_value_bootstrap = np.mean([slope <= 0 for slope in bootstrap_slopes])

# Fit the model on the original data for comparison
original_model = smf.ols('duration ~ waiting', data=old_faithful[long_wait]).fit()
p_value_original = original_model.pvalues['waiting']
...
print(f"Original p-value from smf.ols: {p_value_original:.4f}")
...
# our null hypothesis is no difference between groups on average 
# Looking at the coefficient of C(kind, Treatment(reference="short"))[T.long] is between the 95% confidence interval.
# The P>|t| is 0.000.
# Both have evidence to strongly reject the Null Hypothesis.
...
# Model 3 suggests the plausibility of the assumption that the distribution of error terms is normal. As it shows a smother curve that closely follows the normal distribution line. The other model are either uneven spread(1,2) or too spread out(4)
...
# Assuming `old_faithful` has columns `duration` and `kind` (with 'short' and 'long' categories)
...
# Step 2: Set up permutation test parameters
num_permutations = 10000  # Number of permutations
permuted_diffs = []

# Step 3: Perform the permutation test
for _ in range(num_permutations):
    # Shuffle the 'kind' labels and assign them to a new column
    old_faithful_shuffled = old_faithful.assign(kind_shuffled=old_faithful['kind'].sample(n=len(old_faithful), replace=False).values)
    
    # Calculate mean difference for this shuffle
    permuted_diff = old_faithful_shuffled.groupby('kind_shuffled')['duration'].mean().iloc[::-1].diff().values[1]
    permuted_diffs.append(permuted_diff)

# Step 4: Convert list to numpy array for easier manipulation
permuted_diffs = np.array(permuted_diffs)

# Step 5: Calculate p-value: Proportion of permuted differences that are as extreme as the observed difference
...
# Perform the two-sample bootstrap procedure
...
# For the shuffling we assume there is no difference between the groups, we use any natural association between the label and the duration by randomly shuffling the labels. We then shuffled labels and calculated he difference in means between the new larger and shorter groups, then repeated it multiple times. Then we find the mean difference in this distribution.  
# 
# For bootstrap we are randomly selecting durations from each group and replacing if repeat, then calculate the mean for the bootstrapped samples and place them in the histogram. After repeating for many times we calculate the mean of the histogram and take the 2.5th and 97.5th percentiles of this distribution.

# # similar: 
# 1. They all test for eruption durations between short and long wait times.
# 2. shuffling and the question 11 both provide p-value 
# 3. All test uses means for both groups to find the hypothesis testing or interval estimation.
# 
# # different:
# 1. There ways of processing is different.
# 2. The things they're testing for is different, question 11 is testing for significant difference in means between groups, shuffling is testing significant difference by simulating a null hypothesis with no group difference and bootstrap Estimates a confidence interval for the difference in means and range of the difference. 
# 3. They also provide different output , question 11 provides a p-value and a regression coefficient to test for significance, shuffling provides a p-value indicating whether the observed difference is statistically significant and bootstrap provides the confidence interval.
